...
public class DefaultRequestProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand getKVConfig(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        String value = this.namesrvController.getKvConfigManager()
    ...
    public RemotingCommand registerBrokerWithFilterServer(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        RegisterBrokerResult result = this.namesrvController.getRouteInfoManager()
                .registerBroker(requestHeader.getClusterName(), requestHeader.getBrokerAddr(),
                        ...
                        registerBrokerBody.getTopicConfigSerializeWrapper(), registerBrokerBody.getFilterServerList(),
                        ctx.channel());
    ...
    public RemotingCommand unregisterBroker(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        this.namesrvController.getRouteInfoManager()
                .unregisterBroker(requestHeader.getClusterName(), requestHeader.getBrokerAddr(),
    ...
    public RemotingCommand getRouteInfoByTopic(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        //调用 RouterInfoManager 的方法，从路由表 topicQueueTable/brokerAddrTable/filterServerTable 中分别填充 TopicRouteData 中的
        ...
        if (topicRouteData != null) {
            //如果找到主题对应的路有消息并且该主题为顺序消息，则从 NameServerKvConfig 中获取关于顺序消息相关的配置填充路由消息。
    ...
    private RemotingCommand wipeWritePermOfBroker(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        log.info("wipe write perm of broker[{}], client: {}, {}", requestHeader.getBrokerName(),
                RemotingHelper.parseChannelRemoteAddr(ctx.channel()), wipeTopicCnt);
    ...
    private RemotingCommand getKVListByNamespace(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        byte[] jsonValue =
                this.namesrvController.getKvConfigManager().getKVListByNamespace(requestHeader.getNamespace());
        if (null != jsonValue) {
            response.setBody(jsonValue);
            response.setCode(ResponseCode.SUCCESS);
            response.setRemark(null);
