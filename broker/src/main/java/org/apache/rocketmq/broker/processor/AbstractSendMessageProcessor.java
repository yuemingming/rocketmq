...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    protected RemotingCommand msgCheck(final ChannelHandlerContext ctx, final SendMessageRequestHeader requestHeader,
        ...
        //检查该 Topic 是否可以进行消息发送。主要针对默认主题，默认主题不能发送消息，仅供路由查找
        ...
        //在 NameServer 存储主题的配置信息，默认路径:&{ROCKET_HOME}/store/config/topic.json
        // .下面是主题存储的信息。order：是否是顺序消息；perm：权限码；readQueueNums：读队列数量；writeQueueNums：写队列数量；topicName：主题名称；topicSysFlag
        // ：topic Flag，topicFilterType：主题过滤方式。
        TopicConfig topicConfig =
        ...
        if (queueIdInt >= idValid) {
            String errorInfo =
                    String.format("request queueId[%d] is illegal, %s Producer: %s", queueIdInt, topicConfig.toString(),
                            RemotingHelper.parseChannelRemoteAddr(ctx.channel()));
