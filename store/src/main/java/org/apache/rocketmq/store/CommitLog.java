...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            //Broker 正常停止再重启时，从倒数第三个文件开始进行恢复，如果不足三个文件，则从第一个文件开始恢复。
            int index = mappedFiles.size() - 3;
            if (index < 0) {
                index = 0;
            }
            //mappedFileOffset 为当前文件已校验通过的 offset，proccessOffset 为 Commitlog 文件已确认的物理偏移量等于 mappedFile
            // .getFileFromOffset 加上 mappedFileOffset
            MappedFile mappedFile = mappedFiles.get(index);
            ByteBuffer byteBuffer = mappedFile.sliceByteBuffer();
            long processOffset = mappedFile.getFileFromOffset();
            long mappedFileOffset = 0;
            while (true) {
                //遍历 Commitlog 文件，每次取出一条消息。如果查找结果为 true 并且消息的长度大于 0 表示消息正确，mappedFileOffset 指针向前移动本条消息长度；如果查找结果为 true
                // 并且消息的长度等于 0，表示已到该消息的末尾，如果还有下一个文件则重置 processOffset，mappedFileOffset 重复，否则跳出循环。
            ...
            //更新 MappedFileQueue 的 flushedWhere 与 committedPosition 指针
    ...
    public DispatchRequest checkMessageAndReturnSize(java.nio.ByteBuffer byteBuffer, final boolean checkCRC,
        ...
        try {
            ...
            if (totalSize != readLength) {
                ...
                log.error("[BUG]read total count not equals msg total size. totalSize={}, readTotalCount={}, " +
                                "bodyLen={}, topicLen={}, propertiesLength={}", totalSize, readLength, bodyLen,
                        topicLen,
                        propertiesLength);
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            //遍历 MappedFile 中的信息，验证消息的合法性，并将消息转发到消息消费队列与索引文件。
            ByteBuffer byteBuffer = mappedFile.sliceByteBuffer();
            long processOffset = mappedFile.getFileFromOffset();
            long mappedFileOffset = 0;
            while (true) {
        ...
        // Commitlog case files are deleted
            ...
            //如果没有找到有效 MappedFile，则设置 commitlog 目录的 flushedWhere、committedWhere 指针都为 0，并销毁消息消费队列文件。
            log.warn("The commitlog files are deleted, and delete the consume queue files");
            this.mappedFileQueue.setFlushedWhere(0);
            this.mappedFileQueue.setCommittedWhere(0);
            this.defaultMessageStore.destroyLogics();
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        //首先判断文件的魔数，如果不是 MESSAGE_MAGIC_CODE，返回 false，表示该文件不符合 commitlog 消息文件的存储格式
        if (magicCode != MESSAGE_MAGIC_CODE) {
            return false;
        }

        long storeTimestamp = byteBuffer.getLong(MessageDecoder.MESSAGE_STORE_TIMESTAMP_POSTION);
        //如果文件中第一条消息的存储时间等于 0，返回 false，说明该消息存储文件中未存储任务消息
        if (0 == storeTimestamp) {
            return false;
        }
        //对比文件第一条消息的时间戳与检测点，文件第一条消息的时间戳小于文件的监测点说明该文件部分消息是可靠的，则从该文件开始恢复。
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            putMessageLock.unlock();//处理完消息追加逻辑后将释放 putMessageLock 锁。
        }
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                ...
                //等待同步于刷盘任务完成，如果超时则返回刷盘错误，刷盘成功后正常返回给调用方。
                boolean flushOK =
    ...
    public PutMessageResult putMessages(final MessageExtBatch messageExtBatch) {
        ...
        if (eclipseTimeInLock > 500) {
            log.warn("[NOTIFYME]putMessages in lock cost time(ms)={}, bodyLength={} AppendMessageResult={}",
                    eclipseTimeInLock, messageExtBatch.getBody().length, result);
        }
    ...
    /**
     * 获取当前 CommitLog 目录的最小偏移量，首先获取目录下的第一个文件，如果改文件可用，则返回改文件的起始偏移量，否则返回下一个文件的起始偏移量。
     *
     * @return
     */
    ...
    /**
     * 根据偏移量与消息长度查找消息。首先根据偏移找到所在的物理偏移量，然后用 offset 与文件长度取余得到在文件内的偏移量，从该偏移量读取 size
     * 长度的内容返回即可。如果只根据消息偏移查找消息，则首先找到文件内的偏移量，然后尝试读取四个字节获取消息实际长度，最后读取指定字节即可。
     *
     * @param offset
     * @param size
     * @return
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                int interval = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getCommitIntervalCommitLog();//CommitRealTimeService 线程间隔时间，默认 200ms

                int commitDataLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getCommitCommitLogLeastPages();//一次提交任务至少包含页数，如果待提交数据不足，小于该参数配置的值，将忽略本次提交任务，默认 4 页。

                int commitDataThoroughInterval = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getCommitCommitLogThoroughInterval();//两次真实提交最大间隔，默认 200ms
                //如果距上次提交间隔超过 commitDataThroughInterval，则本次提交忽略 commitLogLeastPages 参数，也就是如果待提交数据小于指定页数，也执行提交操作。
                ...
                try {
                    //执行提交操作，将待提交数据提交到物理文件的内存映射区，如果返回false ，并不代表提交失败，而是表示有数据提交成功了，唤醒刷盘线程执行刷盘操作。该线程每完成一次提交操作，将等待 200ms
                    // 再继续执行下一次提交任务。
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                boolean flushCommitLogTimed = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .isFlushCommitLogTimed();//默认为 false，表示 await 方法等待；如果为 true，表示使用 Thread.sleep 方法等待

                int interval = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getFlushIntervalCommitLog();//FlushRealTimeService 线程任务运行间隔
                int flushPhysicQueueLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getFlushCommitLogLeastPages();//一次刷写任务至少包含页数，如果待刷写数据不足，小于该参数的配置，将忽略本次任务刷写任务，默认 4 页。

                int flushPhysicQueueThoroughInterval = CommitLog.this.defaultMessageStore.getMessageStoreConfig()
                        .getFlushCommitLogThoroughInterval();//两次真实刷写任务最大间隔，默认 10S。

                boolean printFlushProgress = false;

                // Print flush progress
                //如果距上次提哦啊间隔超过 flushPhysicQueueThoroughInterval,则本次执行刷盘任务将忽略
                // flushPhysicQueueLeastPages，也就是如果代刷写数据小于指定页数也执行写磁盘操作。
                ...
                try {
                    ...
                    long begin = System.currentTimeMillis();
                    //调用 flush 方法将内存中的数据刷写到磁盘，并且更新存储检查点文件的 commitlog 文件的更新时间戳，文件检测点文件的刷盘动作在刷盘消息队列线程中执行，其入口为
    ...
    public static class GroupCommitRequest {
        ...
        private volatile boolean flushOK = false;//刷盘结果，初始化为 false。
        ...
        public boolean waitForFlush(long timeout) {
            try {
                //消息发送线程将消息追加到内存映射文件之后，将同步任务 GroupCommitRequest 提交到 GroupCommitService 线程，然后调用阻塞等待刷盘结果，超时时间默认为 5 秒。
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        private volatile List<GroupCommitRequest> requestsWrite = new ArrayList<GroupCommitRequest>();//同步任务暂存容器

        private volatile List<GroupCommitRequest> requestsRead = new ArrayList<GroupCommitRequest>();
        //GroupCommitService 线程每次处理的 request 容器。
        ...
        /**
         * 由于避免通不刷盘消费任务与其他消息生产者提交任务直接的锁竞争，GroupCommitService 提供读容器与谢荣旗，这两个容器每执行完一次任务后，交互，继续消费任务。
         */
            ...
            synchronized (this.requestsRead) {
                ...
                if (!this.requestsRead.isEmpty()) {
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        //刷盘完成之后，唤醒消息发送线程并通知刷盘结束
                        req.wakeupCustomer(flushOK);
                    }

                    long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                    if (storeTimestamp > 0) {
                        //处理完所有同步刷新任务后，更新刷盘检查点 StoreCheckpoint 中的
                        // physicMsgTimestamp，但并没有执行检察点的刷盘操作，刷盘检查点的刷盘操作将在刷写消息队列文件时触发。
                        CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp);
                    }
        ...
        /**
         * GroupCommitService 组提交线程，每处理一批刷盘请求后如果后续有待刷盘的请求需要处理时，组提交线程会“马不停蹄”的处理下一批；如果没有待处理的任务，则休息 10ms，即每 10ms 空转一次。
         */
        public void run() {
            CommitLog.log.info(this.getServiceName() + " service started");
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 创建全局唯一消息 ID，消息 ID 有 16 个字节，4 字节 IP，3 字节端口号，8 字节消息偏移量
            String msgId = MessageDecoder
                    .createMessageId(this.msgIdMemory, msgInner.getStoreHostBytes(hostHolder), wroteOffset);

            // Record ConsumeQueue information
            // 获取该消息在消息队列的偏移量。CommitLog 中保存了当前所有消息队列的当前写入偏移量
            ...
            //根据消息的长度、主题的长度、属性的长度结合消息存储格式计算消息总长度。
            ...
            // 如果消息加上文件结尾超出了文件最大大小，则返回错误，Broker 会重新创建一个新的文件存储该消息。
            ...
            final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
            // Write messages to the queue buffer 将消息存储到 ByteBuffer 中，然后创建 AppendMessageResult。这里只是将消息存储在 MappedFile
            // 对应的内存映射 buffer 中，并没有刷写到磁盘中，追加到结果。
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            while (messagesByteBuff.hasRemaining()) {
                ...
                if (msgLen > this.maxMessageSize) {
                    CommitLog.log
                            .warn("message size exceeded, msg total size: " + msgLen + ", msg body size: " + bodyLen +
                                    ", maxMessageSize: " + this.maxMessageSize);
                    return new AppendMessageResult(AppendMessageStatus.MESSAGE_SIZE_EXCEEDED);
                }
                totalMsgLen += msgLen;
