...
public class RebalancePushImpl extends RebalanceImpl {
    ...
    private final static long UNLOCK_DELAY_TIME_MILLS =
            Long.parseLong(System.getProperty("rocketmq.client.unlockDelayTimeMills", "20000"));
        ...
        if (this.defaultMQPushConsumerImpl.isConsumeOrderly() &&
            ...
            try {
                if (pq.getLockConsume().tryLock(1000, TimeUnit.MILLISECONDS)) {
                    ...
                    log.warn("[WRONG]mq is consuming, so can not unlock it, {}. maybe hanged for a while, {}", mq,
    ...
    public long computePullFromWhere(MessageQueue mq) {
        ...
        switch (consumeFromWhere) {
            ...
            case CONSUME_FROM_MAX_OFFSET:
                /**
                 * offsetStore.readOffset(mq, ReadOffsetType.READ_FROM_STORE)返回-1 表示该消息队列刚创建。从磁盘中读取到消息队列的消费进度，如果大于 0
                 * 返回即可。如果等于-1，CONSUME_FROM_LAST_OFFSET 模式下获取该消息队列最大当前偏移量。如果小于-1，则表示该消息进度文件中存储了错误的偏移量，返回-1；
                 */
            ...
            /**
             * 从磁盘中存储到消息队列的消费进度，如果大于 0 则直接返回即可。如果等于-1，CONSUME_FROM_FIRST_OFFSET 模式下直接返回
             * 0，从头开始。如果小于-1，则表示该消息进度文件中存储了错误的偏移量，返回-1.
             */
            ...
            /**
             * 从消费者启动的时间戳对应的消费进度开始消费。从磁盘中读取到消息队列的消费进度，如果大于 0 则直接返回即可。如果等于 -1，CONSUME_FROM_TIMESTAMP
             * 模式下回尝试去操作消息存储时间戳为消费者启动时间戳，如果能找到则返回找到的偏移量，否则返回 0.如果小于-1，则表示该消息进度文件中存储了错误的偏移量，返回-1；
             */
