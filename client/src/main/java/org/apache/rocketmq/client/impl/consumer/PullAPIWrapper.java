...
public class PullAPIWrapper {
    ...
    public PullResult processPullResult(final MessageQueue mq, final PullResult pullResult,
        ...
        //从消息的拉取流程知道，消息拉取线程 PullMessageService 默认会使用异步方式从服务器拉取消息，消息消费端会通过 PullAPIWrapper 从响应结果解析出拉取到的消息。如果消息过滤模式为 TAG
        // 模式，并且订阅 TAG 集合不为空，则对消息的 TAG 进行判断，如果集合中包含消息的 TAG 则返回给消费者消费，否则跳过。
        ...
        if (PullStatus.FOUND == pullResult.getPullStatus()) {
            ...
            for (MessageExt msg : msgListFilterAgain) {
                ...
                MessageAccessor
                        .putProperty(msg, MessageConst.PROPERTY_MIN_OFFSET, Long.toString(pullResult.getMinOffset()));
                MessageAccessor
                        .putProperty(msg, MessageConst.PROPERTY_MAX_OFFSET, Long.toString(pullResult.getMaxOffset()));
            }
     ...
     * @param maxNums                    最大拉取消息条数
     * @param sysFlag                    拉取系统标记
     * @param commitOffset               当前 MessageQueue 的消费进度（内存中）
     * @param brokerSuspendMaxTimeMillis 消息拉取过程中语序 Broker 挂起时间，默认 15s
    ...
    public PullResult pullKernelImpl(final MessageQueue mq, final String subExpression, final String expressionType,
        ...
        //根据 BrokerName、BrokerId 从 MQClientInstance 中获取 Broker 地址，在整个 RocketMQ Broker 的部署集群中，相同名称的 Broker 构成主从结构，其
        // BrokerId 会不一样，在每次拉取消息后，会给出一个建议，下次拉取从主节点还是从从节点拉取。
