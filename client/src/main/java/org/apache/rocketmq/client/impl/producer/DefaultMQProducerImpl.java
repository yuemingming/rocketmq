...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void initTransactionEnv() {
        ...
        if (producer.getExecutorService() != null) {
            ...
            this.checkExecutor =
                    new ThreadPoolExecutor(producer.getCheckThreadPoolMinSize(), producer.getCheckThreadPoolMaxSize(),
    ...
    private SendResult sendDefaultImpl(Message msg, final CommunicationMode communicationMode,
        ...
        this.makeSureStateOK();//检查实例状态
        Validators.checkMessage(msg, this.defaultMQProducer);//检查消息是否符合状态

        final long invokeID = random.nextLong();
        long beginTimestampFirst = System.currentTimeMillis();
        long beginTimestampPrev = beginTimestampFirst;
        long endTimestamp = beginTimestampFirst;
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());//获取主题的路由信息
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            boolean callTimeout = false;//是否超时
            MessageQueue mq = null;//最终选择的消息队列
            Exception exception = null;//异常消息
            SendResult sendResult = null;//发送结果
            //消息发送次数，默认为 3 次，异步及 oneway 模式默认一次
    ...
    /**
     * 依据 Topic 获取主题的路由信息，如果本地没有缓存，则向 NameServer 查询改 Topic 的路由信息。
     *
     * @param topic
     * @return
     */
                ...
                switch (communicationMode) {
                    case ASYNC:
                        ...
                        sendResult = this.mQClientFactory.getMQClientAPIImpl()
                                .sendMessage(brokerAddr, mq.getBrokerName(), tmpMessage, requestHeader,
                    ...
                    case SYNC:
                        ...
                        sendResult = this.mQClientFactory.getMQClientAPIImpl()
                                .sendMessage(brokerAddr, mq.getBrokerName(), msg, requestHeader, timeout - costTimeSync,
